...
const PredictionsPage = ({ session }) => {
  ...
  const handleSubmit = async (e, type) => {
    ...
    try {
      ...
      // This fetch call now has the correct syntax and uses your live Render URL.
      const response = await fetch(`https://health-risk-api-afp6.onrender.com/predict/${type}`, {
        method: 'POST',
      ...
      setApiError("Failed to get prediction. Please check backend connectivity.");
    }
        ...
        return (
          <form onSubmit={e => handleSubmit(e, 'diabetes')}>
            ...
            <div className="form-grid">
              <div className="form-group"><label>Age (years)</label><input type="number" name="age_diabetes" value={formData.age_diabetes} onChange={handleInputChange} required /></div>
              <div className="form-group"><label>BMI (kg/m²)</label><input type="number" name="bmi" value={formData.bmi} onChange={handleInputChange} step="0.1" required /></div>
              <div className="form-group"><label>Glucose Level (mg/dL)</label><input type="number" name="glucose" value={formData.glucose} onChange={handleInputChange} required /></div>
              <div className="form-group"><label>Insulin Level (μU/mL)</label><input type="number" name="insulin" value={formData.insulin} onChange={handleInputChange} step="0.1" required /></div>
              <div className="form-group form-group-full"><label>Blood Pressure (mmHg)</label><input type="text" name="blood_pressure" value={formData.blood_pressure} onChange={handleInputChange} placeholder="e.g. 120/80" required /></div>
            </div>
        ...
        return (
          <form onSubmit={e => handleSubmit(e, 'heart')}>
            ...
            <div className="form-grid">
              ...
              <div className="form-group"><label>Serum Cholesterol</label><input type="number" name="chol" value={formData.chol} onChange={handleInputChange} required /></div>
              <div className="form-group"><label>Fasting Blood Sugar</label><select name="fbs" value={formData.fbs} onChange={handleInputChange}><option value="1">True</option><option value="0">False</option></select></div>
              <div className="form-group"><label>Resting ECG</label><select name="restecg" value={formData.restecg} onChange={handleInputChange}><option value="0">Normal</option><option value="1">ST-T abnormal</option><option value="2">Left ventricular hypertrophy</option></select></div>
              <div className="form-group"><label>Max Heart Rate</label><input type="number" name="thalch" value={formData.thalch} onChange={handleInputChange} required /></div>
              ...
              <div className="form-group"><label>Slope</label><select name="slope" value={formData.slope} onChange={handleInputChange}><option value="0">Upsloping</option><option value="1">Flat</option><option value="2">Downsloping</option></select></div>
  ...
  return (
    <div className="predictions-page">
      <div className="page-header">
        ...
        <p>Provide your health information for personalized risk assessment.</p>
      </div>
      <div className="tabs">
        ...
        <Modal isOpen={isResultModalOpen} onRequestClose={handleCloseModal} className="auth-modal" overlayClassName="auth-overlay">
          ...
          {apiError ? (
            ...
            <div className="result-content">
              <p><strong>Outcome:</strong> <span className={`risk-tag ${result.prediction === 1 ? 'high' : 'low'}`}>{result.prediction === 1 ? 'High Risk' : 'Low Risk'}</span></p>
